...
public class ImageAnalysisAppController {
    ...
    @FXML
	void autoContrast() {
		int[] histo = histogram.getHistogram();
			double maxPart = (100.00 * histo[255])/histogram.getPixelCount();
			double minPart =  (100.00 * histo[0])/histogram.getPixelCount();
			if (!((maxPart > 1.00)
					||
					(minPart > 1.00))
			) {
				int brightnessGap = 128 - (histogram.max - histogram.min) / 2;
				brightnessSlider.setValue(brightnessGap);
				brightnessChanged();
				double contrastChange = autoContrastGap(histogram.min, histogram.max);
				contrastSlider.setValue(contrastChange);
				contrastChanged();
			}
	}


	double autoContrastGap(int min, int max) {
		return 255.0 / (max - min) * 1.0;
	}
